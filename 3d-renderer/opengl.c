...
void draw_triangle_gl( POINT *v0_ptr, POINT *v1_ptr, POINT *v2_ptr )
{
    ...
    POINT v1 = *v1_ptr;
    POINT v2 = *v2_ptr;
    int need_v_normals = current_rs.shading_mode == FLAT   || current_rs.shading_mode == PHONG  || current_rs.texturing_mode;
    ...
    if( current_as.renderer == SW_HW )              glVertex3f(v0.position[X] - window_width / 2 - 0.5, v0.position[Y] - window_height / 2 - 0.5, -v0.position[Z]);
    ...
    if( current_as.renderer == SW_HW )              glVertex3f(v1.position[X] - window_width / 2 - 0.5, v1.position[Y] - window_height / 2 - 0.5, -v1.position[Z] );
    ...
    if( current_as.renderer == SW_HW )              glVertex3f(v2.position[X] - window_width / 2 - 0.5, v2.position[Y] - window_height / 2 - 0.5, -v2.position[Z] );
...
void draw_line_gl( POINT *start_ptr, POINT *end_ptr )
{
    ...
    if(current_as.renderer == SW_HW)                    glVertex3f( start.position[X] - window_width / 2 - 0.5, start.position[Y] - window_height / 2 - 0.5, -start.position[Z] );
    ...
    if(current_as.renderer == SW_HW)                    glVertex3f( end.position[X] - window_width / 2 - 0.5, end.position[Y] - window_height / 2 - 0.5, -end.position[Z] );
...
void change_gl_state(void)
{
    ...
    if( current_rs.texturing_mode != TEXTURE_OFF )
    {
        ...
        glTexEnvf       ( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE,  (current_rs.modulation_mode) ? GL_MODULATE : GL_DECAL);
        glHint          ( GL_PERSPECTIVE_CORRECTION_HINT,       (current_rs.perspective_correction)  ? GL_NICEST : GL_FASTEST);
    }
    else
    {
        glDisable       ( GL_TEXTURE_2D );
